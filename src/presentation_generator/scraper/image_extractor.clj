...
(defn- parse-wiki-image [wiki-image-tuple key-tokens]
  (let [image-tokens (-> wiki-image-tuple :text tokenize pos-tag filters/nouns)
...
(defn get-wiki-image [url key-string]
  (let [url# (java.net.URL. url)
        nodes (html/html-resource url#)
        key-tokens (filters/nouns (pos-tag (tokenize key-string)))
...
(defn get-image [url key-string]
  (let [url# (java.net.URL. url)
        nodes (html/html-resource url#)
        img-nodes (html/select nodes [:img ])
        key-words (filters/nouns (pos-tag (tokenize (string/lower-case key-string))))
        key-words# (set (map first key-words))
